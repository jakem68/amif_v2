...
def inform_adafruit(temp):
    if not isNaN(temp):
        try:
            ...
            with open("/mnt/KSJ_MEDIA/thermocouple-aio-logfile.txt","a+") as log:
                log.write("Some error sending to Adafruit, trying again {0}\n".format(datetime.datetime.now()))
            ...
            if errorcount > adafruit_send_retries:
                ...
                with open("/mnt/KSJ_MEDIA/thermocouple-aio-logfile.txt","a+") as log:
                    log.write("10 consecutive errors sending to Adafruit, quitting to retry at {0}\n".format(datetime.datetime.now()))
            pass
...
def main():
    ...
    while True:
        ...
        try:
            ...
            with open("/mnt/KSJ_MEDIA/thermocouple-aio-logfile.txt","a+") as log:
                log.write("couldn't inform adafrtuit at {0}\n".format(datetime.datetime.now()))
        try:
            if time_to_inform_opcua():
                inform_opcua(myvar, temp)
        except:
            with open("/mnt/KSJ_MEDIA/thermocouple-aio-logfile.txt","a+") as log:
                log.write("couldn't inform OPCUA server at {0}\n".format(datetime.datetime.now()))
